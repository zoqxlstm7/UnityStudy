...
{
    public const int charBufferSize = 256; // 할당해오는 사이즈 최대 크기
}
...
public class TableRecordParser<TMarshalStruct>
{
    ...
    public TMarshalStruct ParseRecordLine(string line)
    {
        ...
        int structBytesIndex = 0;   // Byte배열을 돌기위한 인덱스 값
        ...
        for (int i = 0; i < fieldInfos.Length; i++)
        {
            ...
            //{
            //    structBytes[structBytesIndex++] = fieldBytes[index];
            //}
    ...
    protected void MakeBytesByFieldType(out byte[] fieldByte, Type dataType, string splite)
    {
        ...
        {
            fieldByte = new byte[MarshalTableConstant.charBufferSize];  // 마샬링을 하기위해서 고정크기 버퍼 생성
    ...
    /// <param name="bytes">마샬링할 데이터가 저장된 바이트 배열</param>
